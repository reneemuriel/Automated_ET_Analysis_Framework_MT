...
def analyse(list_tuple: list, output_path: str, pixel_distance: int, all_actions: list, template_sequence, sequence_comp: bool, kcoeff_analysis: bool,  mean_kcoeff_all, stdev_kcoeff_all, algrthm) -> tuple:

    log.info("Starting action-based analysis.") 
    ...
    # create df that stores all metrics summary dfs per action of all groups
    ...
    for i in range(len(groups)):
        ...
        for j in range(len(participants[i])):
            ...
            for k in range(len(trial_paths[i][j])):
                ...
                ## df with average duration per action (general analysis)

                # one column with duration
    ...
    log.info("Finished action-based analysis.") 
